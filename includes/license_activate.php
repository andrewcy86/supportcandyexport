...
if(isset($license_data['success']) && $license_data['success']){
  update_option('wpsc_export_ticket_license_key',$license_key);
  update_option('wpsc_export_ticket_license_expiry',$license_data['expires']);
  $response = array(
    'success' => 1,
    'messege' => 'License activated successfully!'
  );
} else {
  update_option('wpsc_export_ticket_license_key','');
  update_option('wpsc_export_ticket_license_expiry','');
  $response = array(
    'success' => 0,
