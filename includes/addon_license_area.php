...
<div class="col-md-4 wpsc_addon_license_container">
  <div class="row wpsc_lic_title"><h4>Export Ticket</h4></div>
  <div class="row wpsc_lic_body">
    <input type="text" id="wpsc_export_ticket_license_key" <?php echo $license_expiry ? 'disabled' : ''?> value="<?php echo $license_key?>">
    <?php
    if ($license_expiry) {
      ?>
      <button onclick="wpsc_export_ticket_lic_deactivate();" type="button" name="button">Deactivate</button>
      <?php
    } else {
      ?>
      <button onclick="wpsc_export_ticket_lic_activate();" type="button" name="button">Activate</button>
      <?php
    }
    ?>
  </div>
...
<script>
  function wpsc_export_ticket_lic_activate(){
    var license_key = jQuery('#wpsc_export_ticket_license_key').val().trim();
    if (!license_key) {
      alert('Please enter license key!');
      return;
    }
    jQuery('#wpsc_export_ticket_license_key, .wpsc_lic_body button').attr('disabled',true);
    var data = {
      action: 'wpsc_export_ticket_activate_license',
  ...
  function wpsc_export_ticket_lic_deactivate(){
    var license_key = jQuery('#wpsc_export_ticket_license_key').val().trim();
    if (!license_key) {
      alert('Please enter license key!');
      return;
    ...
    jQuery('#wpsc_export_ticket_license_key, .wpsc_lic_body button').attr('disabled',true);
    var data = {
      action: 'wpsc_export_ticket_deactivate_license',
      license_key : license_key
    };
    jQuery.post(wpsc_admin.ajax_url, data, function(response_str) {
      var response = JSON.parse(response_str);
